...
function plural(n: number, s: string, p: string) { return n === 1 ? s : p; }
...
async function exportInventoryCSV(token: string, categories: Category[]) {
  ...
  for (const cat of categories) {
    let items = cat.items;
    if (items.length === 0 && cat.item_count > 0) {
      items = await apiFetch("/categories/" + String(cat.id) + "/items", {
...
export default function InventoryPanel({ token, isAdmin, onDataChanged }: { token: string; isAdmin: boolean; onDataChanged?: () => void }) {
  ...
  useModalLock(myReqOpen);

  const loadMyRequests = async () => {
    setMyReqLoading(true);
    try {
      const data = await apiFetch('/requests', { headers: { Authorization: `Bearer ${token}` } });
      setMyRequests(data);
    } finally {
  ...
  const loadStats = async () => {
    try {
      setStatsError(null);
      const s = await apiFetch('/stats', { headers: { Authorization: `Bearer ${token}` } });
      setStats({ totalQuantity: Number(s?.totalQuantity || 0), newItems: Number(s?.newItems || 0), usedItems: Number(s?.usedItems || 0) });
    } catch (e: any) {
  ...
  const loadItems = async (categoryId: number) => {
    const items = await apiFetch(`/categories/${categoryId}/items`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const loadCategories = async (preserveExpandedIds: number[] = []) => {
    ...
    try {
      ...
      const next: Category[] = (data || []).map((c: any) => ({
        ...
        total_quantity: c.total_quantity, expanded: preserveExpandedIds.includes(c.id), items: [],
      }));
      setCategories(next);
      const allIds = next.map((c: Category) => c.id);
      for (const id of allIds) await loadItems(id);
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleCategory = async (categoryId: number) => {
    ...
    if (!cat) return;
    if (!cat.expanded && cat.items.length === 0) await loadItems(categoryId);
  ...
  const handleExport = async () => {
    setExporting(true);
    try { await exportInventoryCSV(token, categories); showToast('Inventário exportado!'); }
    catch { showToast('Erro ao exportar.', 'error'); }
    finally { setExporting(false); }
  };

  useEffect(() => {
    (async () => { await Promise.all([loadCategories(), loadStats()]); })();
  ...
  const filtered = useMemo(() => {
    ...
    if (!isAdmin) result = result.filter((c) => c.total_quantity > 0);
    if (q) result = result.filter((c) => c.name.toLowerCase().includes(q));
    return result;
  }, [categories, query, isAdmin]);

  const openCreate = (cat: Category) => { setUpsertMode('create'); setUpsertCategory({ id: cat.id, name: cat.name }); setEditingItem(null); setUpsertOpen(true); };
  const openEdit = (cat: Category, item: Item) => { setUpsertMode('edit'); setUpsertCategory({ id: cat.id, name: cat.name }); setEditingItem(item); setUpsertOpen(true); };
  const askDelete = (item: Item) => { setDeleteItem(item); setDeleteOpen(true); };

  const doDelete = async () => {
    if (!deleteItem) return;
    setDeleteBusy(true);
    try {
      await apiFetch(`/items/${deleteItem.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      setDeleteOpen(false); setDeleteItem(null);
      await refreshAll();
      showToast('Item removido com sucesso!');
    } catch { setDeleteOpen(false); showToast('Erro ao remover item.', 'error'); }
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-6 pt-6">
        ...
        {statsError && (
          <div className="mb-4 rounded-2xl border border-red-500/25 bg-red-500/10 px-4 py-3 text-sm text-red-200">Erro ao carregar métricas: {statsError}</div>
        )}
        ...
        <div className="rounded-2xl bg-white/5 border border-white/10 p-4 mb-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div>
            ...
            <div className="text-xs text-white/50">Expanda para ver itens {isAdmin ? 'e registrar saída' : 'e solicitar retirada'}</div>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={refreshAll} className="px-4 py-2.5 rounded-xl bg-white/8 border border-white/10 hover:bg-white/12 transition text-sm" title="Recarregar">↻</button>
          </div>
        </div>
          ...
          {filtered.map((c) => (
            <div key={c.id} className="rounded-2xl bg-white/5 border border-white/10 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4">
                <button onClick={() => toggleCategory(c.id)} className="flex-1 flex items-center justify-between hover:bg-white/0 transition text-left">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-white/50">
                        {c.items.length > 0
                          ? (() => {
                            const visible = c.items.filter((it) => isAdmin || it.quantity > 0);
                            const qty = visible.reduce((s, it) => s + it.quantity, 0);
                            return `${visible.length} ${plural(visible.length, 'item', 'itens')} • ${qty} ${plural(qty, 'unidade', 'unidades')}`;
                          })()
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs px-2 py-1 rounded-full bg-white/6 border border-white/10 text-white/70">{c.total_quantity} un</span>
                    <div className="text-sm text-white/60">{c.expanded ? '▲' : '▼'}</div>
                  </div>
                </button>
                {isAdmin && (
                  <button onClick={() => openCreate(c)} className="ml-3 px-3 py-2 rounded-xl bg-[rgb(var(--brand))]/20 border border-[rgb(var(--brand))]/30 text-[rgb(var(--brand))] hover:bg-[rgb(var(--brand))]/28 transition text-sm font-semibold">
                    + Adicionar
                  </button>
                )}
              </div>
                  ...
                  {c.items.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {c.items.filter((it) => isAdmin || it.quantity > 0).map((it) => (
                            <tr key={it.id} className="border-b border-white/5 hover:bg-white/5">
                              ...
                              <td className="px-5 py-3">
                                <div className="flex flex-wrap gap-2">
                                  {isAdmin && it.quantity > 0 && (
                                    <button onClick={() => { setSaidaItem(it); setSaidaOpen(true); }}
                                  ...
                                  {isAdmin && it.quantity === 0 && (
                                    <button onClick={() => { setReporItem(it); setReporOpen(true); }}
                                      className="px-3 py-1.5 rounded-lg bg-[rgb(var(--brand))]/15 border border-[rgb(var(--brand))]/30 text-[rgb(var(--brand))] hover:bg-[rgb(var(--brand))]/25 transition text-xs">
                                      Repor
                                    </button>
                                  )}
                                  {isAdmin && (
                                    <>
                                      {it.quantity > 0 && (
                                        <button onClick={() => openEdit(c, it)} className="px-3 py-1.5 rounded-lg bg-white/6 border border-white/10 text-white/80 hover:bg-white/10 transition text-xs">Editar</button>
                                      )}
                                      <button onClick={() => askDelete(it)} className="px-3 py-1.5 rounded-lg bg-red-500/15 border border-red-500/30 text-red-200 hover:bg-red-500/25 transition text-xs">Deletar</button>
                                    </>
                                  )}
                                  {!isAdmin && (
                                    <button onClick={() => { setRequestItem(it); setRequestCategory(c); setRequestOpen(true); }}
                                      className="px-3 py-1.5 rounded-lg bg-[rgb(var(--brand))]/15 border border-[rgb(var(--brand))]/30 text-[rgb(var(--brand))] hover:bg-[rgb(var(--brand))]/25 transition text-xs">
                                      Solicitar
                                    </button>
                                  )}
          ...
          <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#0b0f0c] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
              <div>
                ...
                <div className="text-xs text-white/50">Histórico dos seus pedidos de retirada</div>
              </div>
              <button onClick={() => setMyReqOpen(false)} className="px-3 py-1.5 rounded-lg bg-white/6 border border-white/10 text-white/80 text-xs hover:bg-white/10">Fechar</button>
            </div>
            <div className="overflow-y-auto max-h-[60vh]">
              {myReqLoading ? (
                <div className="p-6 text-white/50">Carregando…</div>
              ) : myRequests.length === 0 ? (
                <div className="p-6 text-white/50 text-sm">Você ainda não fez nenhuma solicitação.</div>
              ) : (
                    ...
                    <div key={r.id} className="px-5 py-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <div className="font-medium text-sm">{r.item_brand} {r.item_model}</div>
      ...
      <SaidaModal open={saidaOpen} onClose={() => setSaidaOpen(false)} item={saidaItem} token={token}
        onSuccess={() => { refreshAll(); showToast('Saída registrada com sucesso!'); }} />
      <ItemUpsertModal open={upsertOpen} mode={upsertMode} token={token} categoryId={upsertCategory?.id || 0}
        categoryName={upsertCategory?.name || ''} initialItem={editingItem}
        onClose={() => setUpsertOpen(false)} onSuccess={refreshAll} onToast={showToast} />
      <ConfirmModal open={deleteOpen} title="Remover item do inventário?" description="O item será apagado permanentemente — Marca, modelo e quantidade. Essa ação não pode ser desfeita."
        confirmText="Remover" danger busy={deleteBusy} onCancel={() => setDeleteOpen(false)} onConfirm={doDelete} />
      <RequestModal open={requestOpen} item={requestItem} categoryName={requestCategory?.name || ''}
        token={token} onClose={() => setRequestOpen(false)}
